...
//test15
	...
	public void renderSkillHUD(RenderGameOverlayEvent event, Locomotive rcCar) {
		...
		if (rcCar.mtcStatus == 1 || rcCar.mtcStatus == 2) {
			 ...
			 */
			int width = this.game.fontRendererObj.getStringWidth("Speed Limit: " + rcCar.speedLimit + " km/h");

			int width2 = this.game.fontRendererObj.getStringWidth("");
			int height = this.game.fontRendererObj.FONT_HEIGHT;
			int padding = 2;
			int margin = 4;
			int xPos = 0 + margin;
			int yPos = 0 + margin;
			...
			if (rcCar.atoStatus == 1) {
				this.drawString(this.game.fontRendererObj, "Speed Limit: " + rcCar.speedLimit + " km/h" + ", ATO on", xPos + 4, yPos, 14737632);
			} else {
				this.drawString(this.game.fontRendererObj, "Speed Limit: " + rcCar.speedLimit + " km/h", xPos + 4, yPos, 14737632);
			}
			this.drawString(this.game.fontRendererObj, "Next Speed Limit: " + rcCar.nextSpeedLimit + " km/h", xPos + 4, yPos + 10, 14737632);
			...
			if (rcCar.xFromStopPoint != 0 && rcCar.yFromStopPoint != 0 && rcCar.zFromStopPoint != 0) {
				this.drawString(this.game.fontRendererObj, "Stop in " + Math.round(rcCar.distanceFromStopPoint)  + " blocks.", xPos + 4, yPos + 19, 14737632);
			}
			if (rcCar.xFromStopPoint == 0 && rcCar.yFromStopPoint == 0 && rcCar.zFromStopPoint == 0 && rcCar.xStationStop != 0 && rcCar.yStationStop != 0  && rcCar.xStationStop != 0 ) {
				this.drawString(this.game.fontRendererObj, "Station stop in " +Math.round(rcCar.distanceFromStationStop) + " blocks.", xPos + 4, yPos + 19, 14737632);
			}
			if (rcCar.xSpeedLimitChange != 0 && rcCar.ySpeedLimitChange != 0 && rcCar.zSpeedLimitChange != 0) {
				this.drawString(this.game.fontRendererObj, "Next speed limit in " + Math.round(rcCar.distanceFromSpeedChange)  + " blocks.", xPos + 4, yPos + 28, 14737632);
			}
