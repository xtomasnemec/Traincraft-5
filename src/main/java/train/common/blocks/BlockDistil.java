...
public class BlockDistil extends BlockContainer {
	...
	public IIcon getIcon(IBlockAccess worldAccess, int i, int j, int k, int side) {
		if (((TileEntityDistil) worldAccess.getTileEntity(i, j, k)).getFacing() != null) {
			side = TileHelper.getOrientationFromSide(((TileEntityDistil) worldAccess.getTileEntity(i, j, k)).getFacing(), EnumFacing.getHorizontal(side)).ordinal();
		}
		if (!this.isActive) {
	...
	public void randomDisplayTick(World world, BlockPos pos,IBlockState state, Random random) {
		if (this.isActive) {
			...
			for (int t = 0; t < 10; t++) {
				...
				world.spawnParticle(EnumParticleTypes.MOB_APPEARANCE, var7, (double) pos.getY() + 1F, var9, 0, 0, 0);
			}
	...
	public static void updateDistilBlockState(boolean flag, World world, int i, int j, int k) {
		//int l = world.getBlockMetadata(i, j, k);//todo: probably unnecessary,
		TileEntity tileentity = world.getTileEntity(new BlockPos(i, j, k));
		keepDistilInventory = true;
		if (flag) {
			world.setBlockState(new BlockPos(i, j, k), BlockIDs.distilActive.block.getDefaultState());
		}
		else {
			world.setBlockState(new BlockPos(i, j, k), BlockIDs.distilIdle.block.getDefaultState());
		}
		keepDistilInventory = false;
		//world.setBlockMetadataWithNotify(i, j, k, l, 2);
		if (tileentity != null) {
			tileentity.validate();
			world.setTileEntity(new BlockPos(i, j, k), tileentity);
		}
	}
