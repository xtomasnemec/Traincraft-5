...
public abstract class DieselTrain extends Locomotive implements IFluidHandler {
	...
	public void onUpdate() {
		...
		if (!worldObj.isRemote) {
			if (theTank.getFluidAmount() != Integer.parseInt(this.dataWatcher.getWatchableObjectString(23).split("-_-")[1])){
				...
				this.dataWatcher.updateObject(28, theTank.getFluid()!=null?theTank.getFluid():0);
			}
	...
	protected void updateFuelTrain(int amount) {
		if (!this.isLocoTurnedOn()) {
			...
			blocksToCheck = new TileEntity[]{worldObj.getTileEntity(TraincraftUtil.getPos(posX, posY -1,posZ)),
					worldObj.getTileEntity(TraincraftUtil.getPos(posX, posY + 2,posZ)),
					worldObj.getTileEntity(TraincraftUtil.getPos(posX, posY + 3,posZ)),
					worldObj.getTileEntity(TraincraftUtil.getPos(posX, posY + 4,posZ))
			};
				...
				if (drain == null && block instanceof IFluidHandler) {
					for (EnumFacing direction : EnumFacing.values()) {
						if (((IFluidHandler) block).drain(direction, 100, false) != null &&
								(getFluid()==null || ((IFluidHandler) block).drain(direction, 100, false).getFluid()==getTank().getFluid().getFluid()) &&
			...
			if(drain==null && cartLinked1 instanceof LiquidTank && !(cartLinked1 instanceof EntityBUnitEMDF7) && !(cartLinked1 instanceof EntityBUnitEMDF3) && !(cartLinked1 instanceof EntityBUnitDD35)){
				if (getFluid() == null) {
					drain = ((LiquidTank) cartLinked1).drain(EnumFacing.DOWN, new FluidStack(LiquidManager.DIESEL, 100), true);
					if (drain == null){
						drain = ((LiquidTank) cartLinked1).drain(EnumFacing.DOWN, new FluidStack(LiquidManager.REFINED_FUEL, 50), true);
					}
				...
				if (getFluid() == null) {
					drain = ((LiquidTank) cartLinked2).drain(EnumFacing.DOWN, new FluidStack(LiquidManager.DIESEL, 100), true);
					if (drain == null){
						drain = ((LiquidTank) cartLinked2).drain(EnumFacing.DOWN, new FluidStack(LiquidManager.REFINED_FUEL, 50), true);
					}
				} else if (getFluid().getFluid() == LiquidManager.DIESEL) {
					drain = ((LiquidTank) cartLinked2).drain(EnumFacing.DOWN, new FluidStack(LiquidManager.DIESEL, 100), true);
				} else {
					drain = ((LiquidTank) cartLinked2).drain(EnumFacing.DOWN, new FluidStack(LiquidManager.REFINED_FUEL, 100), true);
				}
			}
			if (drain != null){
		...
		if (fuelTrain >1 && this.isLocoTurnedOn()) {
			...
			if (fuelTrain < 0) {
				fuelTrain = 0;
				drain(EnumFacing.DOWN, amount, true);
