...
//	public double rotationYawClient;
	...
	private AxisAlignedBB boundingBox = new AxisAlignedBB(0,0,0,1,2,1);

	public static final DataParameter<Integer> FUEL = EntityDataManager.<Integer>createKey(Entity.class, DataSerializers.VARINT);
	...
	public AxisAlignedBB getCollisionBox(Entity entity) {
		return entity.getCollisionBox(this);
	}
	...
	public void pressKey(int i) {
		...
		if (i == 9) {
			if (this.isBeingRidden() && (this.getPassengers().get(0) instanceof EntityPlayer)&&bombTimer<=0) {
				if(this.zeppInvent!=null && this.zeppInvent.length>0){
					for(int t=0;t<this.zeppInvent.length;t++){
						if(this.zeppInvent[t]!=null && this.zeppInvent[t].getItem()!=null && this.zeppInvent[t].getItem() == Item.getItemFromBlock(Blocks.TNT)){
							...
							this.world.playSound((EntityPlayer) this.getPassengers().get(0), new BlockPos(posX,posY,posZ), SoundEvents.ENTITY_TNT_PRIMED, SoundCategory.NEUTRAL, 1.0F, 1.0F);
							bombTimer=100;
							if(this.zeppInvent[t].getCount()-1==0)this.zeppInvent[t]=null;
							return;
						}
					}
				}
	...
	public void onUpdate() {
		...
		for (int j = 0; j < i; j++) {
			...
			AxisAlignedBB axisalignedbb = new AxisAlignedBB(boundingBox.minX, d4, boundingBox.minZ, boundingBox.maxX, d8, boundingBox.maxZ);
		...
		if (this.world.isRemote && this.field_70279_a) {
			if (this.boatPosRotationIncrements > 0) {
				...
				var12 = CommonUtil.wrapAngleTo180(this.boatYaw - this.rotationYaw);
		...
		if (this.isBeingRidden() && this.getPassengers().get(0) instanceof EntityLivingBase) {
		...
		if (rand.nextInt(4) == 0 && fuel > 0) {
			fuel = this.dataManager.get(FUEL);
			fuel--;
			this.dataManager.set(FUEL, fuel);
		}
		double d3 = d * 2D - 1.0D;
		...
		double d12 = CommonUtil.wrapAngleTo180(rot - this.rotationYaw);
		...
		this.setRotation(this.rotationYaw, this.rotationPitch);
		if (updateTicks % 10 == 0) {
//			Traincraft.rotationChannel.sendToAllAround(new PacketZeppelinRotation(this, rotationYaw, roll), new NetworkRegistry.TargetPoint(world.provider.getDimension(), posX, posY, posZ, 400D));
			updateTicks=0;
		}
	}
	...
	public void updateRidden() {
		if (isBeingRidden()) {
			...
			getPassengers().get(0).setPosition(posX + d, posY + getMountedYOffset() + getPassengers().get(0).getYOffset(), posZ + d1);
	...
	protected void readEntityFromNBT(NBTTagCompound nbttagcompound) {
		this.dataManager.set(FUEL, nbttagcompound.getInteger("Fuel"));
	...
	public ItemStack removeStackFromSlot(int index) {
		...
		if (index<zeppInvent.length){
			slot=zeppInvent[index].copy();
			zeppInvent[index]=ItemStack.EMPTY;
		}
		return slot;
	}
	...
	public String getName(){
		...
		return CommonUtil.translate("entity." + s + ".name");
	}
}